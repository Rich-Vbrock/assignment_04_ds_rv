...
ggplot(lobster_changes, aes(x = year, y = total_count, group = site))+
  ...
  scale_y_continuous(limits = c(0, 1000),
    ...
    labs(color = "Marine Protected Area (MPA)",
         ...
         caption = "Figure 1: Annual change in spiny lobster population between the five Santa Barbara coastline data collection sites (Isla Vista, Naples, Carpinteria, Mohawk, and Arroyo Quemado) from 2012 to 2018. MPA sites are delineated with red lines and non-MPA sites are delineated with blue lines.")+
...
- Actual difference in mean size: `r round(mean_nonMPA_2012 - mean_MPA_2012, 2)` mm
...
- Mean lobster size differed significantly between 2018 and 2012 for MPA sites (t(`r round(diff_size_MPA$parameter, 2)`) = `r round(diff_size_MPA$statistic, 2)`, *p* < 0.05).
- Actual difference in mean size: `r round(mean_MPA_2018 - mean_MPA_2012, 2)` mm
- % change: 
- 95% Confidence Interval

##### 2012 vs. 2018 for non-MPAs only (Question 4):
- Mean lobster size did *not* differ significantly between 2018 and 2012 for non-MPA sites (t(`r round(diff_size_non_MPA$parameter, 2)`) = `r round(diff_size_non_MPA$statistic, 2)`, *p* > 0.05).
